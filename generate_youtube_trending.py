...
# 1. Determine RULES_REPO_DIR dynamically
if os.path.exists(os.path.join(SCRIPT_DIR, "youtube_trending.json")) or os.path.basename(SCRIPT_DIR) == "social-saver-rules":
    RULES_REPO_DIR = SCRIPT_DIR
else:
    LOCAL_RULES_DIR = "/Users/gouravsinghal/AndroidStudioProjects/social-saver-rules"
    if os.path.exists(LOCAL_RULES_DIR):
        RULES_REPO_DIR = LOCAL_RULES_DIR
    else:
        RULES_REPO_DIR = SCRIPT_DIR

RULES_JSON_PATH = os.path.join(RULES_REPO_DIR, "youtube_trending.json")

# 2. Determine LOCAL_ASSET_PATH dynamically
...
if not os.path.exists(os.path.dirname(LOCAL_ASSET_PATH)):
    rel_asset_path = os.path.abspath(os.path.join(SCRIPT_DIR, "..", "app", "src", "main", "assets", "youtube_trending_default.json"))
    if os.path.exists(os.path.dirname(rel_asset_path)):
        LOCAL_ASSET_PATH = rel_asset_path
...
def get_next_version(filepath=None, min_version=4.0):
    if filepath is None:
...
def harvest_catchy_shorts():
    ...
    if os.path.exists(RULES_REPO_DIR):
        ...
        if os.path.abspath(__file__) != os.path.abspath(rules_script_path) and os.path.exists(os.path.dirname(rules_script_path)):
            try:
                with open(__file__, "r", encoding="utf-8") as src, open(rules_script_path, "w", encoding="utf-8") as dst:
                    dst.write(src.read())
            except Exception as e:
                print(f"Notice syncing script: {e}", flush=True)
    ...
    # 3. Push to git if running locally outside CI (CI will handle git commit/push via workflow)
    if os.path.exists(RULES_REPO_DIR) and not os.environ.get("GITHUB_ACTIONS"):
